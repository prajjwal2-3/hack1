...
export default function Header({ isAuthenticated }) {
  ...
  return (
    <div className='w-full h-20 p-4 flex items-center justify-between bg-navbar shadow-md:0 1px 3px 0 '>
      ...
      {isAuthenticated?(
        <>
       <button className='bg-yellow-400 px-4 py-2 rounded-lg' onClick={handleLogout}>Sign Out</button>
        </>
      ):(
<>
<Link to="/login"><Button className={'text-white px-4 py-2'} text={'Login'}/></Link>
        <Link to="/register"><Button className={'bg-yellow-400 px-4 py-2 rounded-lg'} text={'Sign Up'} /></Link>
</>
      )}
      </section>
